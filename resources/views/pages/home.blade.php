            ...
            @if (session('success'))
                <div class="alert alert-success d-flex justify-content-between align-items-center">
                    ...
                    @if (session('whatsappLink'))
                        <a href="{{ session('whatsappLink') }}" target="_blank"
                            class="btn btn-outline btn-outline-dashed btn-outline-success btn-active-light-success ms-3">
                                    ...
                                    @if ($user->hasRole(['admin', 'management']) || ($user->hasRole('cabincrew') && $request->user_id == $user->id))
                                        <div class="border border-dashed border-gray-300 rounded px-7 py-3 mb-6">
                                            <div class="d-flex flex-stack mb-3">
                                                <div class="me-3">
                                                    <img src="{{ asset('images/' . $request->product->image) }}"
                                                        class="w-50px ms-n1 me-1" alt="" />
                                                    <a
                                                        class="text-gray-800 text-hover-primary fw-bold">{{ $request->product->product_name }}</a>
                                                </div>
                                            </div>
                                            <div class="d-flex flex-stack">
                                                <span class="text-gray-500 fw-bold">To:
                                                    <a
                                                        class="text-gray-800 text-hover-primary fw-bold">{{ $request->user->name }}</a></span>
                        ...
                        <div class="card-header pt-7">
                            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                                ...
                                <input type="text" data-kt-ecommerce-edit-order-filter="search"
                                    class="form-control form-control-solid w-200 w-lg-200 ps-12"
                                    ...
                                    @foreach (Auth::user()->getRoleNames() as $role)
                                            ...
                                            <div class="position-relative">
                                                @if (Auth::user()->phone_number)
                                                    <a class="btn btn-primary er fs-6 px-8 py-4" data-bs-toggle="modal"
                                                        data-bs-target="#kt_modal_bidding1">Buy Item</a>
                                                    <a class="btn btn-primary er fs-6 px-8 py-4" data-bs-toggle="modal"
                                                        data-bs-target="#kt_modal_bidding">Request Item</a>
                                                @else
                                                    <button class="btn btn-primary er fs-6 px-8 py-4" disabled>Request
                                                        Item</button>
                                        ...
                                        @elseif($role == 'admin')
                                            <a href="{{ route('products.create') }}"
                                                class="btn btn-primary er fs-6 px-8 py-4">Add Item</a>
                                        @endif
                                    @endforeach
                                </div>
                            </div>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <a class="symbol symbol-50px">
                                                        <span class="symbol-label"
                                                            style="background-image:url({{ asset('images/' . $product->image) }});"></span>
                                                    </a>
                                                    <div class="ms-5">
                ...
                <div class="card card-flush" id="card-flush">
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" data-kt-ecommerce-order-filter="search"
                                    class="form-control form-control-solid w-250px ps-12" placeholder="Search Request" />
                            </div>
                        </div>
                        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                            ...
                            @if (auth()->check() && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('management')))
                                <div>
                                    <a href="{{ route('requests.exportExcel') }}" class="btn btn-primary">Export to
                                        Excel</a>
                                    <a href="{{ route('requests.exportPdf') }}" class="btn btn-secondary">Export to
                                        PDF</a>
                                </div>
                            @endif
                        </div>
                                ...
                                @foreach ($requests as $request)
                                    @if ($user->hasRole(['admin', 'management']) || ($user->hasRole('cabincrew') && $request->user_id == $user->id))
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                                        @if (auth()->check() && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('management')))
                                                            <a
                                                                href="{{ route('user.profile', ['userId' => $request->user->id]) }}">
                                                            @else
                                                                <a>
                                                        @endif
                                                        <div class="symbol-label">
                                                            <img src="{{ asset('storage/' . $request->user->profile_picture) }}"
                                                                alt="User Profile" class="w-100" />
                                                        </div>
                                                        </a>
                                                    </div>
                                                    <div class="ms-5">
                                            ...
                                            <td class="text-end">
                                                <span
                                                    class="fw-bold">{{ Carbon::createFromFormat('Y-m-d H:i:s', $request->created_at)->format('d/m/Y') }}</span>
                                            </td>
                                            <td class="text-end">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a data-bs-toggle="modal"
                                                            data-bs-target="#kt_modal_select_users_{{ $request->id }}"
                                                    ...
                                                    @foreach (Auth::user()->getRoleNames() as $role)
                                                        @if ($role == 'cabincrew' && $request->status == 'Processed')
                                                            <div class="menu-item px-3">
                                                                <form
                                                                    action="{{ route('requests.updateStatus', ['requestId' => $request->id, 'status' => 'Completed']) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    <button type="submit"
                                                                        class="menu-link px-3">Accepted</button>
                                                                </form>
                                                            </div>
                                                        @elseif($role == 'admin' && $request->status == 'Need Confirm')
                                                            <div class="menu-item px-3">
                                                                <form
                                                                    action="{{ route('requests.updateStatus', ['requestId' => $request->id, 'status' => 'Processed']) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    <button type="submit"
                                                                        class="menu-link px-3">Confirm</button>
                                                                </form>
                                                            </div>
                                                            <div class="menu-item px-3">
                                                                <form
                                                                    action="{{ route('requests.updateStatus', ['requestId' => $request->id, 'status' => 'Rejected']) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    <button type="submit"
                                                                        class="menu-link px-3">Reject</button>
                                                                </form>
                                                            </div>
                                                        @endif
                                                    @endforeach
                                    ...
                                    <div class="modal fade" id="kt_modal_select_users_{{ $request->id }}"
                                                            ...
                                                            <div class="d-flex flex-stack pb-3">
                                                                <div class="d-flex">
                                                                    ...
                                                                    <div class="ms-5">
                                                                        <div class="d-flex align-items-center">
                                                                            <a href="{{ route('user.profile', ['userId' => $request->user->id]) }}"
                                                                                class="text-gray-900 fw-bold text-hover-primary fs-5 me-4">{{ $request->user->name }}</a>
                                                                            @if ($request->status == 'Completed')
                                                                                <span
                                                                                    class="badge badge-light-success d-flex align-items-center fs-8 fw-semibold">
                                                                                    <i
                                                                                        class="badge py-3 px-4 fs-7 badge-light-success"></i>{{ $request->status }}</span>
                                                                            @elseif($request->status == 'Rejected')
                                                                                <span
                                                                                    class="badge badge-light-danger d-flex align-items-center fs-8 fw-semibold">
                                                                                    <i
                                                                                        class="badge py-3 px-4 fs-7 badge-light-danger"></i>{{ $request->status }}</span>
                                                                            @elseif($request->status == 'Processed')
                                                                                <span
                                                                                    class="badge badge-light-primary d-flex align-items-center fs-8 fw-semibold">
                                                                                    <i
                                                                                        class="badge py-3 px-4 fs-7 badge-light-primary"></i>{{ $request->status }}</span>
                                                                            @else
                                                                                <span
                                                                                    class="badge badge-light-warning d-flex align-items-center fs-8 fw-semibold">
                                                                                    <i
                                                                                        class="badge py-3 px-4 fs-7 badge-light-warning"></i>{{ $request->status }}</span>
                                                                            @endif
                                                                        </div>
                                                                        @foreach ($request->user->getRoleNames() as $role)
                                                                            <span
                                                                                class="text-muted fw-semibold mb-3">{{ $role }}</span>
                                                                        @endforeach
                                                                    </div>
                                                                </div>
                                                                <div clas="d-flex">
                                                                    <div class="text-end pb-3">
                                                                        <span
                                                                            class="text-gray-900 fw-bold fs-5">{{ $request->description }}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="p-0">
                                                                <div class="d-flex flex-column">
                                                                    <div
                                                                        ...
                                                                        <a class="symbol symbol-50px">
                                                                            <span class="symbol-label"
                                                                                style="background-image:url({{ asset('images/' . $request->product->image) }});"></span>
                                                                        </a>
                                                                        <div class="ms-5">
                                                                            <a
                                                                                class="text-gray-800 text-hover-primary fs-5 fw-bold">{{ $request->product->product_name }}</a>
                                                                            <div class="text-muted fs-7">
                                                                                {{ $request->product->id }}</div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="d-flex flex-column">
                                                                    ...
                                                                    <div class="d-flex flex-stack">
                                                                        <div class="d-flex flex-column mw-200px">
                                                                            <div class="d-flex align-items-center mb-2">
                                                                                <span class="text-muted fs-8">Date
                                                                                    Request</span>
                                                                            </div>
                                                                            <div class="d-flex align-items-center mb-2">
                                                                                <span
                                                                                    class="text-gray-900 fw-bold fs-5">{{ Carbon::createFromFormat('Y-m-d H:i:s', $request->created_at)->format('d/m/Y') }}</span>
                                                                            </div>
                                                                        </div>
                                                                        <div class="d-flex flex-column mw-200px">
                                                                            <div class="d-flex align-items-center mb-2">
                                                                                <span
                                                                                    class="text-muted fs-8">Quantity</span>
                                                                            </div>
                                                                            <div class="d-flex align-items-center mb-2">
                                                                                <span
                                                                                    class="text-gray-900 fw-bold fs-5">{{ $request->quantity }}</span>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content rounded">
                ...
                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                    <form id="kt_modal_bidding_form" class="form" action="{{ route('products.requestInventory') }}"
                        method="POST">
                        @csrf
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <div class="" data-kt-modal-bidding-type="dollar">
                                <select id="product-select" name="product_id" aria-label="Select a product"
                                    ...
                                    @foreach ($products as $product)
                                        @if ($product->quantity > 0)
                                            <option value="{{ $product->id }}"
                                                ...
                                                <b>{{ $product->id }}</b>&nbsp;-&nbsp;{{ $product->product_name }}</b>&nbsp;-&nbsp;Size
                                                {{ $product->size }}&nbsp;-&nbsp;Stock {{ $product->quantity }}
                                            </option>
                                        @endif
                                    @endforeach
                                </select>
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <div class="">
                                <input type="text" name="description"
                                    class="form-control form-control-solid form-control-lg" value="Unpaid" readonly>
                            </div>
                        </div>
                        <div class="d-flex flex-column mb-8 fv-row">
                            ...
                            <input type="number" name="quantity" class="form-control form-control-solid form-control-lg"
                                value="1" max="2" autocomplete="off">
                        </div>
                        <div class="text-center">
                            <button href="{{ route('dashboard') }}" class="btn btn-light me-3"
    ...
    <div class="modal fade" id="kt_modal_bidding1" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content rounded">
                ...
                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                    <form id="kt_modal_bidding1_form" class="form" action="{{ route('products.buyItem') }}"
                        method="POST">
                        @csrf
                        <div class="mb-13 text-center">
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <div class="" data-kt-modal-bidding-type="dollar">
                                <select name="product_id" aria-label="Select a Currency"
                                    data-placeholder="Select a currency.."
                                    class="form-select form-select-solid form-select-lg">
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}">
                                            <b>{{ $product->id }}</b> - {{ $product->product_name }} - Size
                                            {{ $product->size }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <div>
                                <input type="text" name="description"
                                    class="form-control form-control-solid form-control-lg" value="Paid" readonly>
                            </div>
                        </div>
                        <div class="fv-row mb-8">
                            <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                            ...
                            <div>
                                <input type="number" name="quantity"
                                    class="form-control form-control-solid form-control-lg" value="1" max="2"
                                    autocomplete="off">
                            </div>
                        </div>
                        <div class="text-center">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            productSelect.addEventListener('change', function() {
                ...
                quantityInput.value = ''; // Reset the quantity input value
            });
